...
async function getImages(appResourcesDirectoryPath) {
	...
	imagesMetadata.forEach((metadata) => {
		if (filteredImagesMetadata[metadata.basename] == null || filteredImagesMetadata[metadata.basename].scale < metadata.scale) {
			filteredImagesMetadata[metadata.basename] = metadata;
		}
	});

	return Object.keys(filteredImagesMetadata)
		.sort()
		.map((key) => filteredImagesMetadata[key]);
}
...
async function getImagesPathsList(resourcesPath) {
	try {
		...
		if (error.code === 'ENOENT') {
			// No images folder created yet
			fs.mkdir(resourcesPath, { recursive: true }, (error) => {
				if (error) throw new Error(`Unable to create images folder. (${error.message})`);
			});
			return [];
		}
