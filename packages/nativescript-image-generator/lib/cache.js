...
function getPlatformCacheFolderPath(platformsDir) {
	return path.join(platformsDir, 'tempPlugin/groupsosimple/nativescript-image-generator');
}
...
async function getPlatformCache(platformsDir, platform) {
	...
	try {
		...
		for (const image of cache.images) {
			...
			if (image.filename == null || image.basename == null || image.hash == null || image.scale == null || cache.output[image.basename] == null || cache.output[image.basename].length === 0) {
				image.dirty = true;
				continue;
			}
